...
DROP PROCEDURE IF EXISTS log_categorias;
DELIMITER $$;
CREATE PROCEDURE log_categorias(accion_in VARCHAR(20), id INT, nombre VARCHAR(20), user_id_in INT)
BEGIN
    DECLARE tabla_cate VARCHAR(50) DEFAULT 'categorias';
    DECLARE datos_de_db TEXT;


    IF (accion = 'Crear' AND id IS NULL) THEN
        INSERT INTO tbl_logs(accion, tabla, new_data, user_id)
        VALUES (accion_in,
                tabla_cate,
    ...
    ELSEIF (accion = 'Actualizar' AND id IS NOT NULL) THEN
        ...
        INSERT INTO tbl_logs(accion, tabla, new_data, old_data, user_id)
        VALUES (accion_in,
                tabla_cate,
                CONCAT('{name:', name'}'),
                datos_de_db,
    ...
    ELSE
        ...
        INSERT INTO tbl_logs(accion, board, old_data, new_data, user_id)
        VALUES (accion_in, tabla_cate,
                datos_de_db,
                null,
                user_id_in);
    END IF;
